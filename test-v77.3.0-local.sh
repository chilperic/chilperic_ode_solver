#!/usr/bin/env bash
# Foko Lab v77.3.0 local validation runner.
# Usage: ./test-v77.3.0-local.sh [--full] [--serve|--browser] [--skip-install]
set -euo pipefail

EXPECTED_VERSION="77.3.0"
...
for arg in "$@"; do
  case "$arg" in
    ...
    --help|-h)
      echo "Usage: ./test-v77.3.0-local.sh [--full] [--serve|--browser] [--skip-install]"
...
VENV_DIR="${FOKOLAB_VENV_DIR:-${TMPDIR:-/tmp}/fokolab-v77.3.0-venv}"
NPM_CACHE="${FOKOLAB_NPM_CACHE:-${TMPDIR:-/tmp}/fokolab-v77.3.0-npm-cache}"
export NPM_CONFIG_CACHE="$NPM_CACHE"
export PLAYWRIGHT_BROWSERS_PATH="${PLAYWRIGHT_BROWSERS_PATH:-${TMPDIR:-/tmp}/fokolab-v77.3.0-playwright}"
