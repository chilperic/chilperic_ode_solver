# Current interface candidate: FokoLab 79.1

New logo and 30-symbol family, color-coded original labs, and the unchanged V6.17 Design Revision book. **Numerical engine version remains 78.2.0.** Start with [the release record](RELEASE-79.1.md) and [fresh validation](VALIDATION-79.1.md). The previous v79.0 archive was unavailable; this candidate explicitly uses the recoverable core-access release as its baseline.

```bash
python3 scripts/serve-fresh.py
```

The full author PDF is included. Review [publication and personal-information boundaries](PUBLICATION_REVIEW.md) before publishing this source tree or static archive. No live repository was modified.

---

## Retained application documentation
