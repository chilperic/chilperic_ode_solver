...
(function(root){'use strict';
 ...
 function showPage(page,chapter=null,updateURL=true,focus=true){
  ...
  const label='printed page '+B.pageLabels[page-1];
  ...
  const practices=selectedChapter?.practices||[];
  if($('practiceLinks')){
   $('chapterPractice').hidden=!practices.length;
   $('practiceLinks').innerHTML=practices.map(p=>`<li><a href="${esc(pdfURL(p.pdfPage))}" data-book-jump="${p.pdfPage}" target="_blank" rel="noopener">Practice ${esc(p.id)} · ${esc(p.title)} <span class="muted">— solution, p. ${p.printedPage}</span></a></li>`).join('');
  }
  document.documentElement.dataset.bookPart=selectedChapter?.part||'';
  document.dispatchEvent(new Event('foko:book-chapter'));
