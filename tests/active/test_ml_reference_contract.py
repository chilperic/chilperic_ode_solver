...
def test_ml_uses_authored_v72_workspace():
    ...
    assert 'src/v72/ml-workspace.js?v=77.4.1' in page
    assert 'src/core/ml-reference.js?v=77.4.1' in page
    assert 'src/models/ml-presets.js?v=77.4.1' in page
...
def test_sciml_loads_the_pareto_capable_core():
    ...
    assert 'src/core/sindy.js?v=77.4.1' in page
...
def test_current_version_and_port_are_consistent():
    ...
    assert '"version": "77.4.1"' in package
    assert "scripts/serve-fresh.py" in package
    assert "freshPort()" in playwright
    assert '"version":"77.4.1"' in version.replace(" ", "")
