...
let model, currentName = CORE[0], last = null, continuation = [], continuation2D = [];
...
function wire(){
  ...
  $('runContinuation2D')?.addEventListener('click', runContinuation2D);
...
function renderEditor(){
  ...
  const paramOptions = Object.keys(model.params).map(k=>`<option value="${esc(k)}">${esc(k)}</option>`).join('') || '<option value="">none</option>';
  $('steadyContParam').innerHTML = paramOptions;
  if($('steadyContParam2')){ $('steadyContParam2').innerHTML = paramOptions; const keys=Object.keys(model.params); if(keys.length>1) $('steadyContParam2').value=keys[1]; }
...
function eigenSummary(J){
 if(!J?.length) return {label:'unknown', detail:'no Jacobian', trace:NaN, determinant:NaN, maxReal:NaN, imag:0};
 if(J.length===1){ const a=J[0][0]; return {label:a<0?'stable':a>0?'unstable':'neutral', detail:`eigenvalue ${a.toPrecision(3)}`, trace:a, determinant:a, maxReal:a, imag:0}; }
 if(J.length===2){ const tr=J[0][0]+J[1][1], det=J[0][0]*J[1][1]-J[0][1]*J[1][0], disc=tr*tr-4*det;
   if(disc>=0){ const root=Math.sqrt(Math.max(0,disc)); const e1=(tr+root)/2, e2=(tr-root)/2; return {label:(e1<0&&e2<0)?'stable':(e1>0||e2>0)?'unstable':'neutral', detail:`trace ${tr.toPrecision(3)}, det ${det.toPrecision(3)}`, trace:tr, determinant:det, maxReal:Math.max(e1,e2), imag:0, discriminant:disc}; }
   return {label:tr<0?'stable':tr>0?'unstable':'neutral', detail:`spiral pair: trace ${tr.toPrecision(3)}, det ${det.toPrecision(3)}`, trace:tr, determinant:det, maxReal:tr/2, imag:Math.sqrt(-disc)/2, discriminant:disc};
 }
 return {label:'computed', detail:'Jacobian available; detailed eigenvalue classification shown for 1D/2D systems', trace:NaN, determinant:NaN, maxReal:NaN, imag:0};
}
function classifyBifurcation(prev, curr){
 if(!curr || !Number.isFinite(curr.residual) || curr.residual>1e-5) return 'not converged';
 const det=curr.determinant, tr=curr.trace, imag=curr.imag||0, maxReal=curr.maxReal;
 if(Number.isFinite(det) && Math.abs(det)<1e-4) return 'fold candidate';
 if(Number.isFinite(tr) && Math.abs(tr)<1e-3 && imag>1e-4) return 'Hopf candidate';
 if(prev){
   if(Number.isFinite(prev.determinant) && Number.isFinite(det) && prev.determinant*det<0) return 'fold crossing';
   if(Number.isFinite(prev.trace) && Number.isFinite(tr) && prev.trace*tr<0 && imag>1e-4) return 'Hopf crossing';
   if(Number.isFinite(prev.maxReal) && Number.isFinite(maxReal) && prev.maxReal*maxReal<0) return 'stability crossing';
 }
 if(curr.stability==='stable') return 'stable branch';
 if(curr.stability==='unstable') return 'unstable branch';
 return 'regular';
}
function localJacobianClassification(J){ return eigenSummary(J); }
function continuationCode(row){ const c=row?.bifurcation || row?.classification || row?.stability || 'unknown'; if(/Hopf/i.test(c)) return 4; if(/fold/i.test(c)) return 3; if(/crossing/i.test(c)) return 2; if(/unstable/i.test(c)) return 1; if(/stable/i.test(c)) return 0; return -1; }
function classCounts(rows){ return (rows||[]).reduce((m,r)=>{ const k=r.bifurcation || r.classification || r.stability || (r.converged?'regular':'not converged'); m[k]=(m[k]||0)+1; return m; },{}); }
...
function setRunState(running, msg){ const b=$('solveSteady'); const c=$('runContinuation'); const d=$('runContinuation2D'); if(b){ b.disabled=running; b.textContent=running?'Solving…':'▶ Solve'; } if(c) c.disabled=running; if(d) d.disabled=running; if($('steadyProgress')) $('steadyProgress').style.width=running?'40%':'0%'; if($('steadyStatus')) $('steadyStatus').textContent=msg; }
function solveCurrent(){ if($('solveSteady')?.disabled) return; try{ readEditor(); FokoSession?.save?.('steady', model); const check=window.FokoModelValidator?.validate?.(model,'steady'); if(check && check.blockers.length) throw new Error(window.FokoModelValidator.message(check)); }catch(e){ showError(e); return; } setRunState(true,'Solving algebraic system through compute bus…'); const t0=performance.now(); const bus=window.FokoComputeBus; const job={id:'steady-solve-'+Date.now(), type:'steady-solve', payload:{model}, progress:p=>{ if($('steadyProgress')) $('steadyProgress').style.width=Math.max(10,Math.min(95,Number(p.progress||0)*100)).toFixed(0)+'%'; if($('steadyStatus')) $('steadyStatus').textContent=p.text||'Solving…'; }}; steadyBusJob=job.id; const exec=()=>{ const out=newton(model.vars.map(v=>Number(v[1]))); out.runtimeMs=performance.now()-t0; out.stability=localJacobianClassification(out.J); out.alternatives=findAlternatives(out); out.computeBus={routed:true,type:job.type,jobId:job.id}; return out; }; const promise=bus?.runLocal?bus.runLocal(job,exec):new Promise((resolve,reject)=>setTimeout(()=>{try{resolve(exec());}catch(e){reject(e);}},20)); promise.then(out=>{ last=out; continuation=[]; continuation2D=[]; updateStatus(); renderPlots(); renderDiagnostics(); setPanel('plotsPanel'); setRunState(false,last.converged?'Solved via compute bus.':'Approximate solution via compute bus.'); }).catch(showError).finally(()=>{ steadyBusJob=null; }); }
function continuationRow(paramValue, params, guess, previous){
 const r=newton(guess, params);
 const eig=eigenSummary(r.J);
 const row={param:paramValue,residual:r.norm,converged:r.converged,stability:eig.label,trace:eig.trace,determinant:eig.determinant,maxReal:eig.maxReal,imag:eig.imag};
 model.vars.forEach((v,i)=>row[v[0]]=r.x[i]);
 row.bifurcation=classifyBifurcation(previous,row);
 return {row, x:r.x};
}
function runContinuation(){ if(!model || $('runContinuation')?.disabled) return; const param=$('steadyContParam').value; if(!param){ $('steadyStatus').textContent='No continuation parameter available.'; return; } setRunState(true,'Running continuation through compute bus…'); const t0=performance.now(); const bus=window.FokoComputeBus; const job={id:'steady-continuation-'+Date.now(), type:'steady-continuation', payload:{model,param}, progress:p=>{ if($('steadyProgress')) $('steadyProgress').style.width=Math.max(10,Math.min(95,Number(p.progress||0)*100)).toFixed(0)+'%'; if($('steadyStatus')) $('steadyStatus').textContent=p.text||'Running continuation…'; }}; steadyBusJob=job.id; const exec=({progress}={})=>{ const rows=[]; const min=Number($('steadyContMin').value), max=Number($('steadyContMax').value), N=Math.max(3,+$('steadyContN').value||30); let guess=last?.x?.slice() || model.vars.map(v=>Number(v[1])); let prev=null; for(let k=0;k<N;k++){ const val=min+(max-min)*(k/(N-1)); const params={...model.params,[param]:val}; try{ const out=continuationRow(val, params, guess, prev); guess=out.x; prev=out.row; rows.push(out.row); }catch(err){ rows.push({param:val,residual:NaN,converged:false,bifurcation:'not converged'}); } if(progress && k%3===0) progress({progress:(k+1)/N,text:'Continuation'}); } return {rows, runtimeMs:performance.now()-t0, computeBus:{routed:true,type:job.type,jobId:job.id}}; }; const promise=bus?.runLocal?bus.runLocal(job,exec):new Promise((resolve,reject)=>setTimeout(()=>{try{resolve(exec({}));}catch(e){reject(e);}},20)); promise.then(out=>{ continuation=out.rows; continuation2D=[]; if($('leftPlotType')) $('leftPlotType').value='continuation'; if($('rightPlotType')) $('rightPlotType').value='classification'; renderPlots(); setPanel('plotsPanel'); const counts=classCounts(continuation); $('steadyDiagnostics').textContent = 'Continuation complete over '+param+'. '+continuation.filter(r=>r.converged).length+'/'+continuation.length+' points converged.\n\nBifurcation/stability classes:\n'+Object.entries(counts).map(([k,v])=>k+': '+v).join('\n'); $('steadyTopStatus').textContent='Continuation'; $('steadyRuntime').textContent=out.runtimeMs.toFixed(1)+' ms'; setRunState(false,'Continuation complete via compute bus.'); }).catch(showError).finally(()=>{ steadyBusJob=null; }); }
function runContinuation2D(){ if(!model || $('runContinuation2D')?.disabled) return; const p1=$('steadyContParam').value, p2=$('steadyContParam2')?.value; if(!p1||!p2||p1===p2){ $('steadyStatus').textContent='Choose two different continuation parameters.'; return; } setRunState(true,'Running 2-parameter continuation through compute bus…'); const t0=performance.now(); const bus=window.FokoComputeBus; const job={id:'steady-continuation-2d-'+Date.now(), type:'steady-continuation-2d', payload:{model,p1,p2}, progress:p=>{ if($('steadyProgress')) $('steadyProgress').style.width=Math.max(10,Math.min(95,Number(p.progress||0)*100)).toFixed(0)+'%'; if($('steadyStatus')) $('steadyStatus').textContent=p.text||'Running 2D map…'; }}; steadyBusJob=job.id; const exec=({progress}={})=>{ const rows=[]; const a0=Number($('steadyContMin').value), a1=Number($('steadyContMax').value), n1=Math.max(3,+$('steadyContN').value||20); const b0=Number($('steadyCont2Min')?.value), b1=Number($('steadyCont2Max')?.value), n2=Math.max(3,+$('steadyCont2N')?.value||20); let base=last?.x?.slice() || model.vars.map(v=>Number(v[1])); let done=0,total=n1*n2; for(let j=0;j<n2;j++){ const v2=b0+(b1-b0)*(j/(n2-1)); let rowGuess=base.slice(); let prev=null; for(let i=0;i<n1;i++){ const v1=a0+(a1-a0)*(i/(n1-1)); const params={...model.params,[p1]:v1,[p2]:v2}; try{ const out=continuationRow(v1, params, rowGuess, prev); rowGuess=out.x; prev=out.row; rows.push({...out.row,param1:v1,param2:v2,param:v1}); }catch(err){ rows.push({param1:v1,param2:v2,residual:NaN,converged:false,bifurcation:'not converged'}); } done++; if(progress && done%5===0) progress({progress:done/total,text:'2D continuation map'}); } } return {rows, runtimeMs:performance.now()-t0, computeBus:{routed:true,type:job.type,jobId:job.id}}; }; const promise=bus?.runLocal?bus.runLocal(job,exec):new Promise((resolve,reject)=>setTimeout(()=>{try{resolve(exec({}));}catch(e){reject(e);}},20)); promise.then(out=>{ continuation2D=out.rows; if($('leftPlotType')) $('leftPlotType').value='continuation-2d'; if($('rightPlotType')) $('rightPlotType').value='classification'; renderPlots(); setPanel('plotsPanel'); const counts=classCounts(continuation2D); $('steadyDiagnostics').textContent = '2-parameter continuation map complete for '+p1+' × '+p2+'. '+continuation2D.filter(r=>r.converged).length+'/'+continuation2D.length+' grid points converged.\n\nClasses:\n'+Object.entries(counts).map(([k,v])=>k+': '+v).join('\n'); $('steadyTopStatus').textContent='2D continuation'; $('steadyRuntime').textContent=out.runtimeMs.toFixed(1)+' ms'; setRunState(false,'2D continuation complete via compute bus.'); }).catch(showError).finally(()=>{ steadyBusJob=null; }); }
...
function renderOne(id,type){ const el=$(id); if(!el) return; if(!last && !continuation.length && !continuation2D.length){ el.innerHTML='<div class="diagnostics empty">Solve to plot.</div>'; return; } const names=model.vars.map(v=>v[0]); if(type==='equilibrium' && last) return safePlot(id,[{x:names,y:last.x,type:'bar',name:'equilibrium'}],layout('Equilibrium values','variable','value'),cfg()); if(type==='residual' && last) return safePlot(id,[{x:last.history.map((_,i)=>i), y:last.history, type:'scatter', mode:'lines+markers', name:'residual'}],{...layout('Newton residual','iteration','||f||'), yaxis:{type:'log',title:'||f||'}},cfg()); if(type==='jacobian' && last?.J?.length) return safePlot(id,[{z:last.J,x:names,y:names,type:'heatmap',colorscale:'Viridis'}],layout('Jacobian matrix','variable','equation'),cfg()); if(type==='stability' && last) return safePlot(id,[{x:['residual','iterations','alternatives','max real eigenvalue'],y:[last.norm,last.iterations,last.alternatives?.length||0,last.stability?.maxReal??0],type:'bar'}],layout('Solve diagnostics','metric','value'),cfg()); if(type==='continuation' && continuation.length){ const traces=names.map(n=>({x:continuation.map(r=>r.param),y:continuation.map(r=>r[n]),mode:'lines+markers',type:'scatter',name:n, text:continuation.map(r=>r.bifurcation)})); return safePlot(id,traces,layout('Continuation branch',$('steadyContParam').value,'equilibrium'),cfg()); } if(type==='continuation-residual' && continuation.length) return safePlot(id,[{x:continuation.map(r=>r.param),y:continuation.map(r=>r.residual),mode:'lines+markers',type:'scatter',name:'residual',text:continuation.map(r=>r.bifurcation)}],layout('Continuation residual',$('steadyContParam').value,'||f||'),cfg()); if(type==='bifurcation' && continuation.length){ const special=continuation.filter(r=>/Hopf|fold|crossing/i.test(r.bifurcation||'')); const traces=[{x:continuation.map(r=>r.param),y:continuation.map(r=>r.maxReal),mode:'lines+markers',type:'scatter',name:'max Re(λ)',text:continuation.map(r=>r.bifurcation)}]; if(special.length) traces.push({x:special.map(r=>r.param),y:special.map(r=>r.maxReal),mode:'markers',type:'scatter',name:'candidate bifurcations',marker:{size:12,symbol:'diamond'},text:special.map(r=>r.bifurcation)}); return safePlot(id,traces,layout('Hopf / fold classification',$('steadyContParam').value,'max Re(λ)'),cfg()); } if(type==='classification' && (continuation.length||continuation2D.length)){ const counts=classCounts(continuation2D.length?continuation2D:continuation); return safePlot(id,[{x:Object.keys(counts),y:Object.values(counts),type:'bar',name:'count'}],layout('Bifurcation and stability classes','class','count'),cfg()); } if(type==='continuation-2d' && continuation2D.length){ const p1=$('steadyContParam').value, p2=$('steadyContParam2')?.value || 'parameter 2'; const xs=[...new Set(continuation2D.map(r=>r.param1))]; const ys=[...new Set(continuation2D.map(r=>r.param2))]; const z=ys.map(y=>xs.map(x=>{ const r=continuation2D.find(q=>Math.abs(q.param1-x)<1e-12 && Math.abs(q.param2-y)<1e-12); return continuationCode(r); })); return safePlot(id,[{x:xs,y:ys,z,type:'heatmap',colorscale:'Viridis',colorbar:{title:'class code'}}],layout('2-parameter continuation map',p1,p2),cfg()); } el.innerHTML='<div class="diagnostics empty">Run the required solve or continuation first.</div>'; }
...
function dataExport(){ return {model,currentName,solution:last,continuation,continuation2D,exportedAt:new Date().toISOString()}; }
