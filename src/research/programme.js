...
(function(root){'use strict';
 ...
 const $=id=>document.getElementById(id),esc=UI.esc,KEY='foko:programme:v6.16';
 let notes=store.get(KEY,{}),current='verify',bookURL='materials/Scientific_Mastery_V6_16.pdf',bookVerified=false;
 ...
 $('bookFile').addEventListener('change',async e=>{
  ...
  try{
   if(root.crypto?.subtle){
    ...
    if(hash!==C.sourceSha256){$('bookStatus').textContent='This is not the supplied V6.16 edition. The existing book remains selected to avoid opening the wrong chapters.';return;}
    bookVerified=true;
   }
   if(bookURL?.startsWith('blob:'))URL.revokeObjectURL(bookURL);bookURL=URL.createObjectURL(file);
   $('bookStatus').textContent=bookVerified?'V6.16 verified. Chapter links open your local file; nothing is uploaded.':'Local PDF selected; edition not verified in this browser. Page links assume V6.16.';
 ...
 document.addEventListener('click',e=>{
  ...
  if(!bookURL){$('bookStatus').textContent='Select your V6.16 PDF above before opening chapter pages.';$('bookFile').focus();UI.toast('Select your local book at the top of the programme.');return;}
