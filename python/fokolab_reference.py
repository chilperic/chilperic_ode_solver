...
# Exported browser downloads append a self-contained request and their own entry point.
if __name__=='__main__' and '__FOKOLAB_EMBEDDED__' not in globals():
    parser=argparse.ArgumentParser(description=__doc__);parser.add_argument('experiment',type=Path);parser.add_argument('--output',default='fokolab_reference_result');args=parser.parse_args()
    save_result(run(json.loads(args.experiment.read_text(encoding='utf-8'))),args.output)
