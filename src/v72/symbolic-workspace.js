(function (root) {
  ...
  function exportJson() {
    ...
    download('fokolab-symbolic-result-v78.2.0.json', JSON.stringify(payload, null, 2), 'application/json');
  }
  function exportCsv() {
    ...
    download('fokolab-symbolic-jacobian-v78.2.0.csv', rows.map(function (row) { return row.join(','); }).join('\n') + '\n', 'text/csv');
  }
  function exportPython() {
    const config = readConfig();
    download('fokolab-symbolic-validation-v78.2.0.py', Core.generateSympyScript(config), 'text/x-python');
  }
  function exportPlot(side, formatName) {
    ...
    root.Plotly.downloadImage(node, { format: formatName, filename: `fokolab-symbolic-${state.plotType[side]}-v78.2.0`, width: 1200, height: 760, scale: 1 });
  }
