(function(){
  ...
  function ensureRuleWorker(){if(customWorker)return customWorker; if(!window.Worker){workerReady=false; return null;} customWorker=new Worker('src/agent-rule-worker.js?v=71.46.0'); customWorker.onerror=e=>{workerReady=false; status('Custom rule worker error: '+(e.message||'unknown error'),'error');}; return customWorker;}
