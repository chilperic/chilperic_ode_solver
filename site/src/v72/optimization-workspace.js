...
(function (root) {
  ...
  function configuration() {
    readEditorIntoModel();
    return { version: '78.2.0', example: state.currentName, model: clone(state.model), settings: readSettings(), layout: state.layout, focusSide: state.focusSide, plotTypes: clone(state.plotTypes) };
  }
  ...
  function serialisableResult() {
    const result = resultForExport();
    return { release: '78.2.0', computedAt: new Date().toISOString(), result, model: state.model, warning: 'Numerical candidate only. Global optimality and KKT sufficiency are not established.' };
  }
