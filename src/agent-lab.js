(function(){
  'use strict';
  // Single source of truth for the build version stamped onto every export.
  // Read from here instead of hard-coding version literals, so Agent and
  // Symbolic labs never drift apart again (see test_v60_build_version_consistency).
  const FOKO_BUILD='v60';
...
return cell;`,life:`// Conway's Game of Life\nif (cell === 1 && (counts.alive === 2 || counts.alive === 3)) return 1;\nif (cell === 0 && counts.alive === 3) return 1;\nreturn 0;`,forest:`// Forest-fire local rule\nif (cell === 0 && rand() < params.A * 0.05) return 1;\nif (cell === 1 && (counts.b > 0 || rand() < params.B * 0.01)) return 2;\nif (cell === 2) return 3;\nif (cell === 3 && rand() > params.D) return 0;\nreturn cell;`}; if(SOCIAL_MODELS.has(kind)) return `// Social / network-contagion rule template\n// 0 outside, 1 baseline/compliant, 2 activated by rumor/hype/panic, 3 corrected/resistant/removed\nconst pressure = (counts.b + 0.6 * counts.c) / Math.max(1, counts.degree);\nif (cell === 1 && rand() < params.A * pressure + params.B * 0.035) return {state:2, event:'contagion'};\nif (cell === 2 && rand() < params.C * 0.08) return {state:3, event:'correction'};\nif (cell === 3 && rand() < params.D * 0.015) return 1;\nreturn cell;`; return byKind[kind]||templates[approach]||templates.rule_based;}
  ...
  function customSkeleton(){return {lab:'Agent Lab',version:FOKO_BUILD,id:'my-custom-agent-model',family:'custom',label:'My custom agent model',caption:'Define state meanings, parameters and a local update rule.',approach:$('agentApproach')?.value||'rule_based',timeMode:$('agentTimeMode')?.value||'discrete_sync',topology:$('agentTopology')?.value||'moore',states:['empty','state 1','state 2','state 3'],colors:['#ffffff','#2563eb','#dc2626','#16a34a'],params:['birth / activation','death / recovery','mutation / import','crowding / memory'],a:25,b:10,c:5,d:2,density:35,initial:{1:32,2:3,3:0},rule:['document your transition rules here'],ruleCode:templateFor('custom')};}
  ...
  function configObject(){return {lab:'Agent Lab',version:FOKO_BUILD,example:state.kind,approach:$('agentApproach')?.value,timeMode:$('agentTimeMode')?.value,topology:$('agentTopology')?.value,dt:dt(),ruleMode:$('agentRuleMode')?.value,customRule:$('agentCustomCode')?.value,customModel:$('agentCustomModelJson')?.value,gridSize:state.n,initialPopulation:initialPopulationCount(),density:initialPopulationFraction(),parameters:params(),initialFractions:initialFractions(),seed:num('agentSeed',2026),step:state.t,plotMode:$('agentPlotMode')?.value,palette:paletteKey(),states:model().states};}
  function exportConfig(){return configObject();}
  function jsonText(){return JSON.stringify(configObject(),null,2);}
  function downloadJson(){const txt=jsonText(); $('agentJsonBox').value=txt; const blob=new Blob([txt],{type:'application/json'}); const url=URL.createObjectURL(blob); const a=document.createElement('a'); a.href=url; a.download='foko-agent-model-'+FOKO_BUILD+'.json'; a.click(); setTimeout(()=>URL.revokeObjectURL(url),1000); status('JSON exported.');}
