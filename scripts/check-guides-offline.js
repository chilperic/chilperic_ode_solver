...
(async () => {
  ...
  try {
    ...
    assert.equal(await page.locator('h1').textContent(), 'Foko Lab modelling handbook');
    assert.ok(await page.locator('[data-guide-section]').count() >= 26);
    assert.ok(await page.locator('.guide-toc-level-3').count() >= 40);
    assert.equal(await page.locator('nav.foko-main-nav').locator(':scope > *').count(), 6);
    ...
    assert.equal(await page.locator('h1').textContent(), 'Practical modelling curriculum');
    assert.equal(await page.locator('.tutorial-complete').count(), 20);
    const first = page.locator('.tutorial-complete').first();
    await first.click();
    assert.equal(await first.getAttribute('aria-pressed'), 'true');
    assert.match(await page.locator('#tutorialProgressLabel').textContent(), /^1 of 20 tutorials completed$/);
    ...
    console.log('Guide offline Chromium contract passed: canonical navigation, searchable handbook chapters, twenty tutorial modules, persistent completion controls and responsive learning UI are functional.');
