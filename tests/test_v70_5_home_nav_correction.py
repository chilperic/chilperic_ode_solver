...
def test_dropdown_links_are_actual_visible_menu_items():
    for path in HTML:
        ...
        assert len(items) == 10, path
        labels = [item.get_text(' ', strip=True) for item in items]
        for expected in ['ODE Lab', 'Stochastic Lab', 'Optimization Lab', 'Steady-State Lab', 'Symbolic Lab', 'Agent Lab', 'Standalone ODE Lab']:
...
def test_release_token_is_v70_5_for_html_assets():
    for path in HTML:
        ...
        assert '?v=70.4.0' not in html, path
        assert '?v=70.7.0' in html, path
